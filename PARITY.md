...
| `POST /crawl` | `tavily-crawl` | `tavily crawl` | shipped (M5) | Inherits all map traversal flags (depth, breadth, limit, filters, allow-external) + extract flags (extract-depth, format, images, favicon, usage, chunks-per-source). URL via positional arg or stdin `-`; `--dry-run` supported. |
...
All four MCP tools (`tavily-search`, `tavily-extract`, `tavily-map`, `tavily-crawl`) are shipped — no MCP-tool gap.
